...
class BaseFormater:
    ...
    @staticmethod
    def data_frame_from_excel(path: str, settings: dict) -> DataFrame:
    ...
    def start_format(self, sheet_settings: dict, file_path: str, file_year: int, save_path: str,
                     ignore_sheet: set, excel_to_csv=None):
        file_name = file_path.split("/")[-1]

        print('\nОткрываем файл ' + file_name)
        for data in sheet_settings.items():
            if data[0] in ignore_sheet:
                continue
            df = excel_to_csv(file_path, file_year, data[1])
            self.save_to_csv(df, 'CSVs/' + data[1]['csv_path'], ';')
            self.save_to_csv(df, save_path + '/result/' + data[1]['csv_path'], ';')


class FullFormater(BaseFormater):
    def excel_to_csv(self, path: str, year: int, settings: dict) -> DataFrame:
    ...
    def start_format(self, sheet_settings: dict, file_path: str, file_year: int, save_path: str, ignore_sheet: set):
...
class UlskFormater(BaseFormater):
    def excel_to_csv(self, path: str, year: int, settings) -> DataFrame:
        print('Обработка ' + settings['sheet'] + '...')
        df = self.data_frame_from_excel(path, settings)
