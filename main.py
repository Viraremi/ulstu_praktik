...
#     "name": 'sheet_name',
#     "columns": [0, 10],
#     "column_from_drop": [0, 1],
#     'raws': [0,10]
# }

def exel_to_csv(path: str,
                sheet: str,
                year: int,
                csv_path: str,
                iloc_rows,
    ...
    df = df.iloc[iloc_rows[0]:iloc_rows[1], iloc_columns[0]:iloc_columns[1]] # Обрезаем лишнее
...
exel_to_csv('data_xlsx/2021x.xlsx', '1.1. Кол-во ГС', 2021, 'CSVs/Amount_GS.csv',
            [6,20], [6, 18],[9, 13, 17], m_id)
...
m_id = pd.MultiIndex.from_product([id_SubFed, group, status], names=['id_SubFed', 'group', 'status'])
exel_to_csv('data_xlsx/2021x.xlsx', '1.2. Кол-во МС', 2021, 'CSVs/Amount_MS.csv',
            [6,20], [6, 14],[9, 13], m_id)
